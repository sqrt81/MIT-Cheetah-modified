...
double SolveQuadProg(const Eigen::MatrixXd& G, Eigen::VectorXd& g0,
                    ...
                    Eigen::VectorXd& x)
{
    ...
    Eigen::LLT<Eigen::MatrixXd> llt_g(n);
    llt_g.compute(G);
    Eigen::MatrixXd J = llt_g.matrixU().solve(Eigen::MatrixXd::Identity(n, n));
...
//    std::cout << "x: " << x.transpose() << std::endl << std::endl;
    ...
    // if use double instead of double.
